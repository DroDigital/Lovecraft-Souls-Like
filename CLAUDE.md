...
- `src/data`: `tuning.ts` holds every tunable number (boss numbers in `bossTuning`, re-exported); `schema`, `roster` (every id), `entities/` (one file per tier), `registry`, `validate`, `archetypes`, `attacks`, `regions`; `moves`, `placeholders` (dummy + Deep One), `arena`; `sites` and `dungeons` (the world's places and room graphs).
...
- `src/ui`: HUD (+ `bossHud`, `hudKit`), debug panel, Elder Sign `signMenu`, `autosave`, `?bestiary`, `?look` look test, orbit rig.
