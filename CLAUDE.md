...
- `src/data`: `tuning.ts` holds every tunable number (boss numbers in `bossTuning`, play numbers in `playTuning`, re-exported); `schema`, `roster` (every id), `entities/` (one file per tier), `registry`, `validate`, `archetypes`, `attacks`, `regions` (+ `regionFeatures`: roads, towns, features); `moves`, `placeholders` (dummy + Deep One), `arena`; `sites`, `dungeons` and `lairs` (the world's places and room graphs); story: `intro`, `npcs`, `quests`, `documents`; `endings`; `sounds` + `voices` (audio recipes) + `music` (boss scores).
