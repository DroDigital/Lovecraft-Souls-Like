...
## Commands
- `npm run dev`: title, then the open world saved to localStorage (click captures the mouse; E rests at an Elder Sign; Esc/Start: pause,
  settings, controls). `?fresh` new game, no title; `?arena` combat arena; `?bestiary` every roster entity (click to fight); `?spawn=<id>
...
- `src/systems`: the simulation, no Three.js. `game.ts` builds the world and fixes the system order; actions + input buffer, movement, combat, revolver, stamina, lock-on, camera rig, archetype `brain` + `perception`, `creatures` (EntityDef → combatant), death/Echoes; the mind: `sanity`, `insight`, hooks `hiddenLayer`, `variantSwap`, `hallucinations`; the world: `overworld`, `population`, `checkpoints` (signs, travel, gates), `save`; bosses: `bossFight` (phases), `specials` + `projectiles` + `hazards` (attack effects), `reality` + `realityTricks` (hooks), `arenaChanges`, `signatures/` (+ Hastur's calling, Nyarlathotep's watching), `fightActions` (E), `endings`.
- `src/render`: pipeline, post pass (grade + character rim), world material, `lantern.ts` (night: moon + lantern), `shaders/`, palette, textures, `fx.ts`; figures + `poses.ts`, actor views, follow camera; `sprites/` (pixel sprite generator + atlas), `assemblies` (colossi), `creatureViews`, `hiddenViews`; `fxController` (sanity FX hook); `worldScene` (chunk streaming) with `terrainMesh`, `propMeshes`, `siteMeshes`, `signMeshes`; `fightViews` (bolts, pools, props, water) + `realityFx`; `rigidBatch` (colossi in ≤3 draws); `audio/` (engine, synth, drones, cues, gameAudio).
- `src/data`: `tuning.ts` holds every tunable number (boss numbers in `bossTuning`, re-exported); `schema`, `roster` (every id), `entities/` (one file per tier), `registry`, `validate`, `archetypes`, `attacks`, `regions`; `moves`, `placeholders` (dummy + Deep One), `arena`; `sites` and `dungeons` (the world's places and room graphs); `endings`; `sounds` + `voices` (audio recipes).
...
- `src/ui`: HUD (+ `bossHud`, `hudKit`), debug panel, `menuKit` (screens, keys/pad nav) for `titleScreen`, `pauseMenu`, `menuPages`, Elder Sign `signMenu`, `endingCard`; `settings`, `autosave`, `?bestiary`, `?look` look test, orbit rig.
