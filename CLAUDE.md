...
- `src/render`: pipeline, post pass (grade + character rim), world material, `lantern.ts` (night: moon + lantern), `shaders/`, palette, textures, `fx.ts`; figures + `poses.ts`, actor views, follow camera; `sprites/` (pixel sprite generator + atlas), `assemblies` (colossi), `creatureViews`, `hiddenViews`; `fxController` (sanity FX hook) + `audioFx`; `worldScene` (chunk streaming) with `terrainMesh`, `propMeshes`, `siteMeshes`, `signMeshes`; `fightViews` (bolts, pools, props, water) + `realityFx`.
- `src/data`: `tuning.ts` holds every tunable number (boss numbers in `bossTuning`, re-exported); `schema`, `roster` (every id), `entities/` (one file per tier), `registry`, `validate`, `archetypes`, `attacks`, `regions`; `moves`, `placeholders` (dummy + Deep One), `arena`; `sites` and `dungeons` (the world's places and room graphs); `endings`.
...
- `src/ui`: HUD (+ `bossHud`, `hudKit`), debug panel, Elder Sign `signMenu`, `endingCard`, `autosave`, `?bestiary`, `?look` look test, orbit rig.
