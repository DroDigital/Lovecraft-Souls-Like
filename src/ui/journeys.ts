 ...
 * waits in the dark while the chunks about it are built at a larger budget; and the veil lifts once
 * they stand, the world moving again as it does. Dying, the veil falls as the investigator does,
 * and the death throes wait for it before the respawn.
 */
...
export interface Journeys {
  ...
  /** The veil already covers (a game begun): names the region the investigator stands in, lifts once the world stands, then calls `then`. */
...
const HOLD_MS = [900, 4000] as const; // under the veil after a jump: at least (the sign shows), and at most while chunks build
const BUILD_MS = 14; // chunk building a frame while nothing shows
const DYING_MS = 900; // the fall shows this long before the veil comes

export function createJourneys(g: Game, veil: Veil): Journeys {
  let phase: 'idle' | 'covering' | 'dying' | 'holding' = 'idle';
  let since = 0;
  ...
  const hold = (after?: () => void): void => {
    [phase, since, then, rise] = ['holding', performance.now(), after, 1];
  };
  ...
  const self: Journeys = {
    ...
    update(pending) {
      ...
      if (phase !== 'holding' || now - since < HOLD_MS[0] || (pending > 0 && now - since < HOLD_MS[1])) return;
      phase = 'idle';
      void veil.lift(1.4);
      const after = then;
      then = undefined;
    ...
    get still() {
      return phase === 'covering' || (phase === 'holding' && rise <= 0) || dyingWaits();
    },
    get budget() {
