 ...
 * is to be done next, those done, and every tome and note read, each of which opens again to be
 * reread.
 */
...
export function journalPage(g: Game, back: () => void, show: (p: Page) => void): Page {
  const page: Page = {
    ...
    build(p) {
      ...
      heading(p, 'UNDER WAY');
      if (!open.length) el(p, 'div', 'Nothing yet. Talk to the people you meet.', 'opacity:.5');
