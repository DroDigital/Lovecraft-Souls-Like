/** Phase 0 look test (`?look`): the §2 render pipeline on the test scene, driven by a sanity slider and an orbit camera. */
...
import { createOrbitRig } from './orbitRig';

export function startLookTest(): void {
  ...
  const panel = createDebugPanel(state, ['drag/wheel orbit · O auto-orbit']);
